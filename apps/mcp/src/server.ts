...
export function createCrmMcpServer(client: CrmClient): McpServer {
  ...
  server.registerTool(
    "list_business_units",
    {
      description: "List enabled Comp CRM business units. Use this before writing leads.",
      inputSchema: {},
    },
  ...
  server.registerTool(
    ...
    async (input) => result(await client.ingestLeads(ingestLeadsInput.parse(input))),
  );
